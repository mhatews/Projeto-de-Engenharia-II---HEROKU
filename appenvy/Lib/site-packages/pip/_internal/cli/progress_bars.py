...
class WindowsMixin:
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        # The Windows terminal does not support the hide/show cur ANSI codes
        # even with colorama. So we'll ensure that hide_cur is False on
        # Windows.
        # This call needs to go before the super() call, so that hide_cur
        # is set in time. The base progress bar class writes the "hide cur"
        # code to the terminal in its init, so if we don't set this soon
        # enough, we get a "hide" with no corresponding "show"...
        if WINDOWS and self.hide_cur:  # type: ignore
            self.hide_cur = False
